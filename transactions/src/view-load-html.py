...
@solara.component
def RegisterCard(account: Account, the_months: dict[str, list[str]]):
                        ...
                        with solara.lab.Tab(str(month)):
                            ...
                            with solara.Column(style="background-color: transparent; height: 500px; overflow: auto; row-gap: 0px;"):
                                for j, (transaction, running_sum) in enumerate(zip(sorted_transactions, running_sums)):
                                    if not transaction.date.startswith(filter_prefix):
                                        continue
                                    with solara.Row(style="align-items: center; background-color: transparent; column-gap: 0px;") as row:
                                        ...
                                        solara.Markdown(f"{transaction.h}", style="color: #999; font-family: monospace; white-space: pre;")
                                        the_months_entry = the_months.get(transaction.h)
                                        solara.Markdown("<br>".join([
                                            f"{transaction.date}  {transaction.payee}",
                                            f"{transaction.amount / 100:10.2f}  {transaction.memo if transaction.memo != "" else "(no memo)"}",
                                            *(
                                                ["UNCATEGORIZED"] if the_months_entry is None
                                                else [f"<strong>[{cp.amount / 100:8.2f}]  " + " > ".join(cp.path) + "</strong>" for cp in the_months_entry.paths] if the_months_entry.is_split == "yes"
                                                else [f"<strong>" + " > ".join(the_months_entry.path) + "</strong>"]
                                            ),
    ...
    with solara.Column(align="center", style="width: 800px; overflow: auto; margin: 0 auto !important;"):
        ...
        with solara.Column(gap="0px", style="width: 800px; overflow: auto;") as col:
            ...
            for i, account in enumerate(root.value.accounts):
                ...
                RegisterCard(account, root.value.months)
